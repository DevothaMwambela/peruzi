...
const getBaseUrl = () => {
    ...
    // Check if we're in the peruzi directory or frontend subdirectory
    if (path.includes('/peruzi/') || path.includes('/frontend/')) {
        return `${origin}/peruzi/api`;
    }
    
    // Default to include peruzi path
    return `${origin}/peruzi/api`;
};
